...
public class Controller implements ActionListener, ChangeListener, MouseMotionListener, MouseListener {
    ...
    private void check(int i)
    {
        ...
        for(int j = 0; j<favorite.getCount(); j++)
        {
            ...
            {
                likeButtons.get(i).setIcon(new ImageIcon("src\\main\\java\\pictures\\yFavorites.png"));
                change = true;
            }
        }
        if(!change)
            likeButtons.get(i).setIcon(new ImageIcon("src\\main\\java\\pictures\\noFavorites.png"));
    }
        ...
        if(FavouritesView)
        {
            ...
            for (int i = 0; i < favorite.getCount(); i++) {
                if (favorite.getSong(i).getFile() ==
                    ...
                    favoriteSongIndex = i;
                    likeButtons.get(index).setIcon(new ImageIcon("src\\main\\java\\pictures\\noFavorites.png"));
                }
            }
            if (favoriteSongIndex == -1) {
                likeButtons.get(index).setIcon(new ImageIcon("src\\main\\java\\pictures\\yFavorites.png"));
    ...
    private void randomButtonAction() throws Exception
    {
        ...
        {
            isRandom = false;
            GUI.getRandomPlayButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\randomButton1.png"));
        }
        else
        {
            isRandom = true;
            GUI.getRandomPlayButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\randomButton.png"));
        }
    }
    ...
    private void loopButtonAction() throws Exception {
        if (!loop) {
            loop = true;
            utils.loop(true);
            GUI.getLoopButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\loop1Button.png"));
        } else {
            loop = false;
            utils.loop(false);
            GUI.getLoopButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\loopButton.png"));
        }
    }
    ...
    private void playButtonAction() throws Exception {
        ...
        if (!isPlaying) {
            ...
            thread.start();
            GUI.getPlayButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\pauseButton.png"));
        } else {
            System.out.println("Stop");
            isPlaying = false;
            utils.stopSound();
            GUI.getPlayButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\playButton.png"));
        }
    ...
    private void speakerButtonAction() throws Exception {
        if (!SliderIsZero) {
            if (!muted) {
                muted = true;
                utils.setMute(true);
                GUI.getSpeakerButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\muted.png"));
            } else {
                muted = false;
                utils.setMute(false);
                GUI.getSpeakerButton().setIcon(new ImageIcon("src\\main\\java\\pictures\\speaker.png"));
            }
        }
    }
